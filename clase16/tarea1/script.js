...
class Rectangulo {
  ...
  actualizaAnimacion() {
    this.ctx += 1 / 4;  // Actualiza solo la animación de este rectángulo
    this.posY -= 0.1;
...
const dibuja = (textura, codigoDeTextura, tras = false, trasTex = false, posTexX = 0, posTexY = 0, esSprite = true, tiempoCompletado = false) => {
  ...
  //requestAnimationFrame(() => dibuja(textura, codigoDeTextura, tras, posX, posY, trasTex, posTexX, posTexY, esSprite));
...
const dibujaEscena = () => {
  ...
  if (tiempoReal > PERIODO_MOVIMIENTO) {
    ...
    dibuja(textura3, codigoDeTextura3, true, true, 0, 0, true, false);
    dibuja(textura1, codigoDeTextura1, true, true, 0, 0, true, false);
  }
